...
function query_users(users::Vector{<:String})
    ...
    output = DataFrame()
    for n in [1:1:length(json.data);]
        println(n)
        soc_a = String[]
        if (eval(Meta.parse(string.("json.data._", n, ".socialAccounts.totalCount"))) > 0)
            for i in [1:1:eval(Meta.parse(string.("json.data._", n, ".socialAccounts.totalCount")));]
                println(i)
                push!( soc_a,  eval(Meta.parse(string.("json.data._", n, ".socialAccounts.edges[", i, "].node.url"))))
            end
        end

        org_a = String[]
        org_b = String[]
        if (eval(Meta.parse(string.("json.data._", n, ".organizations.totalCount"))) > 0)
            for i in [1:1:eval(Meta.parse(string.("json.data._", n, ".organizations.totalCount")));]
                println(i)
                push!( org_a, eval(Meta.parse(string.("json.data._", n, ".organizations.edges[", i, "].node.id"))) )
                push!( org_b, eval(Meta.parse(string.("json.data._", n, ".organizations.edges[", i, "].node.login"))))
            end
        end

        new_output = DataFrame(id = eval(Meta.parse(string.("json.data._", n, ".id"))),
                            bio = eval(Meta.parse(string.("json.data._", n, ".bio"))),
                            company = eval(Meta.parse(string.("json.data._", n, ".company"))),
                            pronouns = eval(Meta.parse(string.("json.data._", n, ".pronouns"))),
                            isDeveloperProgramMember = eval(Meta.parse(string.("json.data._", n, ".isDeveloperProgramMember"))),
                            isEmployee = eval(Meta.parse(string.("json.data._", n, ".isEmployee"))),
                            updatedAt = eval(Meta.parse(string.("json.data._", n, ".updatedAt"))),
                            twitterUsername = eval(Meta.parse(string.("json.data._", n, ".twitterUsername"))),
                            websiteUrl = eval(Meta.parse(string.("json.data._", n, ".websiteUrl"))),
                            socialAccounts = [soc_a],
                            organization_ids = [org_a],
                            organization_logins = [org_b],
                            user_email = eval(Meta.parse(string.("json.data._", n, ".email")))
                            )
