...
function query_users_dev(users::Vector{<:String})
    ...
    if (haskey(user_json, "errors"))
        ...
        # TODO: HACK (fix this) needs to be global so the eval(Meta.parse(string. 's can see this variable
        global user_json = JSON3.read(result.Data)
        ...
        execute(conn, "BEGIN;")
        GHOST.load!(output, conn, "UPDATE $schema.test_usr_codegov SET bio =\$2, company=\$3, pronouns=\$4, isDeveloperProgrammember=\$5, isEmployee=\$6, updatedAt=\$7, twitterUsername=\$8, websiteUrl=\$9, socialAccounts=\$10, organization_ids=\$11, organization_logins=\$12, user_email=\$13 WHERE id = \$1;")
        execute(conn, "COMMIT;")
...
for i in 1:grouped_users_count
    ...
    try
        query_users_dev(grouped_users[i])
    catch
        continue
    end
end
